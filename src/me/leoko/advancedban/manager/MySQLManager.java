...
public class MySQLManager {
	...
	private boolean hasFailed(){
		if(failed) System.out.println("AdvancedBan <> Skipped executing SQL because there is no connection to the MySQL-Server please restart the server!");
		return failed;
	}
	...
	private void connect(){
		...
		if(autoRefresh)
			Universal.get().getMethods().scheduleAsyncRep(new Runnable(){
				@Override
				public void run() {
					reconnect();
				}
			}, 20*60*refreshMin, 20*60*refreshMin);
	}
	...
	public void executeSatement(String sql){
		if(hasFailed()) return;
		try {
	...
	public ResultSet executeRespSatemen(String sql){
